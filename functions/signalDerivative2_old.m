function [outputMtx, RMSE]= signalDerivative(inputMtx, nDerivative, method,freq)
arguments
    inputMtx (3,3,:) double
    nDerivative (1,1) double
    method (1,1) string {mustBeMember(method,["Oh2","Oh4","cspline"])} ="cspline"
    freq (1,1) double=1
end
...
% 1)	derivata prima e seconda direttamente da f(x) con tre modalità:
%     a.	O(h2);
...
% method       [string]       = può assumere solo tre valori, cioè 'Oh2', 'Oh4', 'cspline';
% -------------------OUTPUT---------------------
% outputMtx    [ nFrame x nSignal ] = segnali di uscita disposti per colonne;
% RMSE         [1 x nSignal]        = questo vettore non è [] solo nel caso del metodo spline; in questo caso contiene il RMSE fra inputMtx e la spline cubica generata.
% -------------------ESEMPIO--------------------
